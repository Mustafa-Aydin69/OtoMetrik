// linkler.txt'deki (otomobil) linkleri sirayla isler: her link icin kategori sayfasini "?take=50"
// ile gezip ilanlari data/output/arabam_test_val.csv'ye yazar (CsvWriter.seenIds ile ayni resume
...
// kazima scriptleriyle (scrape-linkler.js, scrape-minivan-panelvan-brands.js) ayni bicimde
// loglar/scrape-log-tum-markalar.txt dosyasina eklenir (append) - marka basi ilan satirlari,
// "tamamlandi" ozeti ve calisma sonunda OZET.
...
// scrape-linkler.js / scrape-minivan-panelvan-brands.js ile ayni bicimde: console'a yazar ve
// ayni satiri loglar/scrape-log-tum-markalar.txt dosyasina ekler (append).
function log(line) {
...
run().catch((err) => {
  console.error('linkler.txt (otomobil) bazli kazima basarisiz:', err);
  process.exit(1);
});
